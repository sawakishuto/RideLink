...
final class EncounterRepository: EncounterRepositoryProtocol {
    ...
    func receptionFriendReqest() -> AnyPublisher<FriendInfoModel, Error> {
        let subject = CurrentValueSubject<FriendInfoModel, Error>(FriendInfoModel(id: "11", isOnline: true, profile: UserProfileModel(userName: "a", bikeName: "a", profileIcon: nil, touringcomment: nil)))
        subject.send((FriendInfoModel(id: "11", isOnline: true, profile: UserProfileModel(userName: "a", bikeName: "a", profileIcon: nil, touringcomment: nil))))
        return subject.eraseToAnyPublisher()
    }
